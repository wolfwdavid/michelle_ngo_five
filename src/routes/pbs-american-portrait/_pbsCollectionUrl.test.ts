...
// embedded in description; 3 do not. Audit-verified ids:
const PBS_NULL_IDS = ['620232398', '1007061884', '1007027015'] as const;

describe('pbsCollectionUrl — 15 positive extractions', () => {
  it('extracts a https://www.pbs.org/american-portrait/collection/... URL from each of the 15 records that contain one', () => {
    ...
    const withUrl = pbs.filter(
      (v) => !PBS_NULL_IDS.includes(v.id as (typeof PBS_NULL_IDS)[number])
    );
    expect(withUrl.length).toBe(15);
    for (const v of withUrl) {
      // Video.description is z.string().optional(); the 18 PBS rows all carry a
      // description, but the type is widened — coalesce to '' so the helper's
      // `string` signature is satisfied.
...
describe('pbsCollectionUrl — returns null for 3 records without URL', () => {
...
describe('pbsCollectionUrl — edge cases', () => {
  it('strips trailing period', () => {
    ...
    expect(url).toBe(
      'https://www.pbs.org/american-portrait/collection/57/i-began-to-live-my-truth/'
    );
  });
  ...
  it('returns null on malformed protocol', () => {
    expect(
      pbsCollectionUrl('See: httpx://www.pbs.org/american-portrait/collection/57/x/')
    ).toBeNull();
  });
});
